...
if [ "$GPU" = true ]; then
    ...
    echo "[GPU] Score compressed (auto dispatch)..."
    cargo bench --bench gpu_score --features gpu 2>/dev/null
    echo ""

    echo "[GPU] Score compressed (forced GPU, QJL_GPU_MIN_BATCH=0)..."
    QJL_GPU_MIN_BATCH=0 cargo bench --bench gpu_score --features gpu 2>/dev/null
    echo ""

    echo "--- GPU Results ---"
    echo ""
    echo "  Score compressed (auto dispatch):"
    print_result "100 vector pairs" "score_compressed_cpu_vs_gpu/cpu/100"
    print_result "1K vector pairs" "score_compressed_cpu_vs_gpu/cpu/1000"
    print_result "10K vector pairs" "score_compressed_cpu_vs_gpu/cpu/10000"
    print_result "100K vector pairs" "score_compressed_cpu_vs_gpu/cpu/100000"
    echo ""
    echo "  Store score_all_pages:"
    print_result "10 pages" "score_all_pages/pages/10"
    print_result "100 pages" "score_all_pages/pages/100"
    print_result "1000 pages" "score_all_pages/pages/1000"
    echo ""
fi
